...
const RegisterForm = ({user}: {user: User}) => {
  ...
  async function onSubmit(values: z.infer<typeof PatientFormValidation>) {
    ...
    // Initialize FormData (key/value pairs) to prepare file data for HTTP requests
    let formData;
    ...
    if(values.identificationDocument && values.identificationDocument.length > 0) {
      ...
      formData.append("blobFile", blobFile); // Append the file blob to FormData under the key "blobFile"
    ...
    try {
      const patientData = {
        ...values,
        //only the values that will change
        userId: user.$id,
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-12 flex-1">
        ...
        <CustomFormField 
          ...
          placeholder="Mother had breast cancer, Father had diabetes"
        />
        <CustomFormField 
          fieldType={FormFieldType.TEXTAREA}
          control={form.control}
